...
local config = {
tt = 2, --默认报告间隔
MaxStaggerTaken = 80, --醉拳承受率上限,高于此比例时提示使用更多活血
...
pfblvl = true, 	--是否报告活血使用水平 如果醉拳承受低于上限值10%以上则提示打的不错.
...
local function PrintTable(name,tab)
	local p =tab.dot/tab.pool		
		p = math.floor(p*1000)/10
				
	print("玩家："..name.."的本次战斗结束","战斗时长："..tab.combatend-tab.combatstart)
	print("醉拳承受："..tab.dot:shortnum(),"醉拳吸收："..tab.pool:shortnum(),"活血率："..p.."%")
	print("被命中次数："..tab.hits,"其中断铁骨被命中次数："..tab.isb,"覆盖率："..(1-tab.isb/tab.hits):percent)
end 
...
local function trigger(self,event,timestamp,eventtype,hideCaster,srcGUID, srcName, srcFlags,srcRFlags,dstGUID,dstName, dstFlags,dstRFlags,...)	
	...
	if eventtype == "SPELL_CAST_SUCCESS" and data[srcName].casts[other[1]] then	--施法次数统计
		if not data[dstName] then  data[dstName]=CreateTable() end --建档
		data[srcName].casts[other[1]] = data[srcName].casts[other[1]] + 1
		if other[1] == spell.pfb and not OnAura(dstName,buff.isb) then --裸活血次数
			data[srcName].purify = data[srcName].purify + 1
			if config.purify then report(dstName.."在断铁骨下使用了"..GetSpellLink(spell.pfb).."请覆盖"..GetSpellLink(spell.isb) ,false,dstName)  end
		end 
	end 
	...
	if eventtype =="SPELL_PERIODIC_DAMAGE"  and other[1] ==spell.dot then	--醉拳打脸				
		...
		if data[dstName].pool>0 then 
			...
			if p > setting.MaxStaggerTaken/100 and other[4]>config.Maxdot then 		--醉拳承受过高
				p=math.floor(p*1000)/10
	...
	if GetTime()>checkpool+3 then  --每3秒检查是否脱战
		...
		for name,tab in pairs(data) do  --遍历表，查看酒仙酒池是否归零
			...
			--开始计算战斗时长，铁骨覆盖率，活血率，技能使用情况
				...
				local str = ""
				tab.combatend = GetTime()	--记录脱战时间
				local p =0
				p = tab.dot/tab.pool
				p = math.floor(p*1000)/10.."%"
				
				if p > setting.MaxStaggerTaken/100 then str = name.."的醉拳承受："..p.."%，请更多使用"..spell.pfb end 
				if p < setting.MaxStaggerTaken/100-0.1 then str = name.."的醉拳承受："..p.."%，打得不错" end
			end 
		end 
	end
	
	if eventtype == "SPELL_AURA_REMOVED" and other[1]==buff.isb and InCombatLockdown()  then 
		if setting.isbfade then report(dstName.."已经失去"..GetSpellLink(spell.isb) .."，治疗注意！",false)  end 
	end	
	if eventtype == "SPELL_AURA_APPLIED" and other[1]==buff.isb and  InCombatLockdown() then 
		if  setting.isbapply  then report(dstName.."已经获得"..GetSpellLink(spell.isb) .."，稳如POI！",false) end 
	end			
